...
### `orioledb.page_checksum`

|             |     |
| ----------- | --- |
| **Default** | on  |

Compute and store page checksums when writing pages to disk. Pages are always
verified on read when they carry a checksum, independent of this setting.
Turning it off only stops newly written pages from getting one.
